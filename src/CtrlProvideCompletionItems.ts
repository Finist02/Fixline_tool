...
export class ProvideCompletionItemsCtrl {
	...
	public setCompletionsStandart(word: string) {
		const matchedKeys = ctrlKnowItems.getMatchedKeys(word);
		for (let i = 0; i < matchedKeys.length; i++) {
			const element = matchedKeys[i];
				var docuStr = matchedKeys[i].toString();
				var snippetCompletion = new vscode.CompletionItem(element.name, matchedKeys[i].getCompletionItemKind());
				snippetCompletion.insertText = new vscode.SnippetString(matchedKeys[i].getSnippetString());
				if (docuStr !== "") {
					snippetCompletion.documentation = new vscode.MarkdownString(docuStr);
				}
				this.completions.push(snippetCompletion);
		}
	}
	public GetSymbols() {
...
export class CtrlCompletionItemProvider implements vscode.CompletionItemProvider {
	private provideCompletionItemsCtrl: ProvideCompletionItemsCtrl;
	...
	public provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
		...
		this.provideCompletionItemsCtrl.setCompletionsStandart(document.getText(document.getWordRangeAtPosition(position)));
